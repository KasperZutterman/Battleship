...
public class Level_4 {
  ...
  private boolean bonusGekregen = false;
  private boolean campaign;

  /*--------------Bord wordt gemaakt--------------*/
  private Bord bordC = new Bord(40, 240, 20, 20, w, h);
  private Bord bordP = new Bord(720, 240, 20, 20, w, h);
  ...
  public Level_4(String _title, boolean _campaign) {  
    title = _title;
    campaign = _campaign;
  ...
  public void act() {
    ...
    text("Punten: " + puntenLevel_4, 320, 860);
    ...
    if (!alleBotenGezet()) {
      ...
      if (!campaign) {
        ...
        if (gewonnen) {
          felicitatiesCampaignGewonnen(screenPage); //opgelet! hier mag niet gevraagd worden om verder te spelen, het is het laatste level...
        } else {
    ...
    //als alle boten van de speler gezet zijn en als alle boten van de computer gezet zijn kan het vuren beginnen
      ...
      //bonus computer bij achterstand van 8 --> extra schot
      if ((hitDoorComputer <= hitDoorSpeler - 8) && (!bonusGekregen)) {
        if (int(random(0,10))<11) {
          int X = int(random(0,20));
          int Y = int(random(0,20));
          while(bordP.coord[X][Y].type != 1) {
            X = int(random(0,20));
            Y = int(random(0,20));
          }
          bordP.coord[X][Y].type = 5;
        } else {
  ...
  *  deze methode staat in voor de schoten die uitgevoerd worden door de computer in de campaign (hunt mode)
  ...
  private void hunt() {
    if (schotenDoorSpeler == (schotenDoorComputer+1)) {
      ...
      potentialT_x = shorten(potentialT_x); //verwijdert de ingeladen coord uit de potential target
      potentialT_y = shorten(potentialT_y); //verwijdert de ingeladen coord uit de potential target
  ...
  // checkt of de potentiele target x en y binnen de range valt en dit vak nog niet is aangeklikt,
  // indien goed, woden deze aan de respectievelijke coords lijst toegevoegd
